...
function writeTreeDecompositionFile(data) {
  const treeDecompositionFile = fs.createWriteStream('dist/tree-decomposition-files/td.td');
...
function writeNiceTreeDecomposition(data) {
  const niceTreeDecompositionFile = fs.createWriteStream('dist/tree-decomposition-files/nicetd.td');
  const splitted = data.split('\n');
