...
export default class Graph {
  ...
  isNodeInTreeDecomposition(node) {
    const nodesInTreeDecomposition = d3
  ...
  getSubGraphLinks() {
    return this.links.filter((currentLink) => {
  ...
  getSubGraphNodes(subTree) {
    ...
    return this.nodes.filter((currentNode) =>
  ...
  colorTreeDecompositionNodes(sourceNode, targetNode) {
    d3.selectAll('#tree-container ellipse')
      .filter(
        (bag) =>
          bag.vertices.includes(sourceNode) && bag.vertices.includes(targetNode)
      )
      .transition()
      .duration(this.animationDuration)
      .delay(this.animationDuration * this.anim)
  ...
  highlightLinksOfSubtree(newBags) {
    d3.selectAll('#tree-container line')
      ...
      .style('stroke', HIGHLIGHTED_LINK)
      .on('end', () =>
        d3.selectAll('#tree-container line').style('stroke', 'black')
      );
  }
  ...
  setBalancedSeparatorExerciseTextCorrect() {
    d3.select('#separator-output').html(
      `${checkmarkSvg()} $S = \\{ ${this.selectedNodes.sort(
        (a, b) => a - b
  ...
  setBalancedSeparatorExerciseTextWrong() {
    d3.select('#separator-output').html(
      `${errorSvg()} $S = \\{ ${this.selectedNodes.sort(
        (a, b) => a - b
  ...
  setIncorrectSeparatorExerciseOutput() {
    ...
    <span>$S = \\{ ${this.selectedNodes.sort(
      (a, b) => a - b
    )} \\}$ is not a separator in the graph</span>
    `);
  }
  ...
  setCorrectSeparatorExerciseOutput() {
    ...
    <p>$S = \\{ ${this.selectedNodes.sort(
      (a, b) => a - b
    )} \\}$ is a separator in the graph</p>
    `);
  }
  ...
  isTree() {
    if (this.isConnected() && this.nodes.length - 1 === this.links.length) {
      return true;
    }
    return false;
  }
  ...
  handleMouseOver(node) {
    ...
    circle.transition().duration(1000).ease(d3.easeElastic).attr('r', 25);
  }

  handleMouseOut(node) {
    const circle = d3.select(`#graph-node-${node.id}`);
    circle.transition().duration(1000).ease(d3.easeElastic).attr('r', 20);
  }
