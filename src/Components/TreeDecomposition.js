...
import { HIGHLIGHTED_NODE, NONHIGHLIGHTED_NODE } from '../constants.js';
...
export default class TreeDecomposition {
  ...
  createTreeDecompositionNodeSvgs() {
    this.nodeSvg = this.svg
      .selectAll('g')
      .append('ellipse')
      .attr('id', (node) => `tree-node-${node.id}`)
      .attr('rx', (d) => d.label.length * 10)
      .attr('original-rx', (d) => d.label.length * 10)
      .attr('ry', 25)
      .attr('class', 'tree-decomposition-node');
  }
  ...
  disableHoverEffect() {
    d3.selectAll('#graph-container circle')
      .transition()
      .duration(100)
      .style('fill', NONHIGHLIGHTED_NODE);
  }
